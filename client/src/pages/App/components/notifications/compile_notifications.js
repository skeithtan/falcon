...
const getNotificationsForAdmin = state => {
    const {
        changeRequests: {
            changeRequests,
            isLoading: changeRequestsIsLoading,
        },
        faculty: {
            faculties,
    ...
    if (changeRequests && faculties) {
        Object.entries(changeRequests).forEach(([facultyId, facultyChangeRequests]) => {
            ...
            const changeRequestCount = facultyChangeRequests.length;
            ...
            notifications.push(onClose => (
                <NotificationItem
                    ...
                    url={makeURL().facultyProfiles().selectFaculty(faculty._id).changeRequests().build()}
                    onClose={onClose}
                />
            ));
...
const getNotificationsForFaculty = state => {
    ...
    if (changeRequests.changeRequests && profile && changeRequests.changeRequests[profile._id]) {
        ...
        // Remove all pending
        myChangeRequests
            .filter(changeRequest => changeRequest.status !== CHANGE_REQUEST_STATUSES.PENDING.identifier)
                    ...
                    <NotificationItem
                        ...
                        renderText={() => (
                            <Typography>
                                ...
                                titled <strong>{changeRequest.title}</strong> has
                                been <strong>{verb}</strong>.
                            </Typography>
                        )}
                        url={makeURL().myProfile().changeRequests().build()}
                        onClose={onClose}
                    />
                ));
...
export const compileNotificationsFromState = state => getObjectForUserType(state.authentication.user, {
